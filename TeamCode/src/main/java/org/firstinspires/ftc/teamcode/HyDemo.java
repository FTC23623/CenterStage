...
@TeleOp(name = "HyDemo")
public class HyDemo extends LinearOpMode {
  ...
  private void ProcessArm() {
    ...
    if (allowManualArmControl) {
      ...
      if (gamepad1.cross) {
        ...
        upperArmPower = gamepad1.right_stick_y;
        lowerArmPower = gamepad1.left_stick_y;
  ...
  private void ProcessIntake(boolean inCassetteFull) {
    ...
    gamepad1RtTrg = gamepad1.right_trigger;
    gamepad1LtTrg = gamepad1.left_trigger;
    // Set the desired motor power and direction based on the user input
    if (gamepad1RtTrg > cTrgBtnThresh) {
      // Right trigger sets the motor to bring pixels in
      intakeDir = cIntakeIn * gamepad1RtTrg;
    } else if (gamepad1LtTrg > cTrgBtnThresh) {
      // Left trigger sets the motor to bring pixels out
      intakeDir = cIntakeOut * gamepad1LtTrg;
    } else {
      intakeDir = cIntakeStop;
    }
    // If the user is running the intake, always run out if the cassette is already full
    if (inCassetteFull && (gamepad1RtTrg > cTrgBtnThresh || gamepad1LtTrg > cTrgBtnThresh)) {
  ...
  private boolean ProcessCassette(int inArmPosition) {
    ...
    if (gamepad1.right_trigger > cTrgBtnThresh) {
    ...
    } else if (cAllowFrontScoreFromCassette && inArmPosition == 4 && gamepad1.right_bumper) {
